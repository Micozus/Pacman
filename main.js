...
class PacmanGame {
    ...
    startingDoors = this.allPositions.filter(
        item => item.char === "S"
    );
    ...
    modifyMovementSpeed(ghost, modifier) {
        let ghostIntervalToChange = this.gameIntervals.ghostMovement.find(
            interval => interval.ghost.id === ghost.id
        );
        clearInterval(ghostIntervalToChange.interval);
        const intervalToSet = setInterval(
            () => this.ghostMovement(ghost),
            100 * modifier
        );
    ...
    checkIfLvlUp() {
        if (this.dots.length === 0 && this.powerups.length === 0 && this.life > 0) {
    ...
    playerReset(ghost) {
        if (this.player.x === ghost.x && this.player.y === ghost.y) {
            ...
            setTimeout(() => this.gameReset("pacmanDeath"), 2600);
        }
    }
    ...
    ghostActivationRoutine() {
        ...
        setTimeout(() => {
            if (!this.elements.ghostPink.activated)
                this.activateGhost(this.elements.ghostPink);
        }, 1000);
        ...
        if (
            ...
            this.activateGhost(this.elements.ghostBlue);
    ...
    moving(elem, direction) {
        ...
        if (elem !== this.player) {
            this.startingDoors.forEach(door => {
                if (elem.x === door.x && elem.y === door.y) {
                    elem.x = currentPosition.x;
                    elem.y = currentPosition.y;
                }
    ...
    pacmanInit() {
        ...
        this.ghosts.forEach(ghost => {
            ...
            ghost.movementInterval = setInterval(
                () => this.ghostMovement(ghost),
    ...
    pacmanDestroyAnimation() {
        if (this.player.alive) {
            ...
            let animation = setInterval(() => {
                this.player.spriteX =
                    destroyStartingTile + startingPosition * this.spriteTile;
                startingPosition++;
    ...
    resetDots() {
        ...
        this.powerups = this.allPositions.filter(
            item => item.char === "P" || item.char === "K"
        );
    }
...
class Canvas {
    ...
    showGameOver() {
        ...
        ctx.font = '20px Consolas';
        ctx.fillText('Press space bar to start a new game', canvas.width / 2, canvas.height / 2 + 30);
    };
    ...
    showCongratulations() {
        ...
        ctx.font = '20px Consolas';
        ctx.fillText('You won life! If you wish to continue - go on, press space bar!', canvas.width / 2, canvas.height / 2 + 30);
    };
    ...
    canvasInit() {
        ...
        const step = () => {
            if (game.gameStarted) {
                ...
                // wyniku
                // S
                let scoreWord = [19, 3, 15, 18, 5];
                scoreWord.forEach((letter, index) => {
                    ctx.drawImage(
                        sprites,
                        2 + letter * 12,
                        24,
                ...
                game.scorePoints.forEach((letter, index) => {
                    ctx.drawImage(
                        sprites,
                        2 + letter * 12,
                        2,
                ...
                let lvlWord = [12, 22, 12];
                lvlWord.forEach((letter, index) => {
                    ctx.drawImage(
                        sprites,
                        2 + letter * 12,
                        24,
        ...
        canvas
            .width = game.map.width * game.map.tile;
        canvas
            .height = game.map.height * (game.map.tile + 1);
        sprites
            .onload = () => {
            window.requestAnimationFrame(step);
        };
...
// STRASZNA IFOLOGIA - DO PRZEPISANIA
const keyEvent = event => {
    event.preventDefault();
    if (game.gameStarted && (event.code === "ArrowRight" || event.code === "ArrowLeft" || event.code === "ArrowUp" || event.code === "ArrowDown")) {
        if ((event.code === "ArrowRight" || event.code === "ArrowLeft" || event.code === "ArrowUp" || event.code === "ArrowDown") && game.player.alive)
            if (game.elements.player.direction === "ArrowDown" || game.elements.player.direction === "ArrowUp") {
                if (event.code === "ArrowDown" || event.code === "ArrowUp") {
                    game.elements.player.direction = event.code;
                }
            }
        if (game.elements.player.direction === "ArrowLeft" || game.elements.player.direction === "ArrowRight"
        ) {
            game.elements.player.direction = event.code;
        }
...
window.addEventListener("keyup", event => keyEvent(event));
